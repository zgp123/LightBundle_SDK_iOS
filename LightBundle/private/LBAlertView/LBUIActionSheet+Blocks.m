...
-(id)initWithTitle:(NSString *)inTitle cancelButtonItem:(LBRIButtonItem *)inCancelButtonItem destructiveButtonItem:(LBRIButtonItem *)inDestructiveItem otherButtonItems:(LBRIButtonItem *)inOtherButtonItems, ...
{
    if((self = [self initWithTitle:inTitle delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:nil]))
    {
        ...
        LBRIButtonItem *eachItem;
        va_list argumentList;
        if (inOtherButtonItems)
        {
            [buttonsArray addObject: inOtherButtonItems];
            va_start(argumentList, inOtherButtonItems);
            while((eachItem = va_arg(argumentList, LBRIButtonItem *)))
            {
        ...
        for(LBRIButtonItem *item in buttonsArray)
        {
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex >= 0)
    {
        ...
        LBRIButtonItem *item = [buttonsArray objectAtIndex:buttonIndex];
        if(item.action)
            item.action();
    }
