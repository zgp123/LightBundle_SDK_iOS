...
@interface LBZXQRCodeAlignmentPatternFinder ()
...
@property (nonatomic, strong, readonly) LBZXIntArray *crossCheckStateCount;
@property (nonatomic, weak, readonly) id<LBZXResultPointCallback> resultPointCallback;

@end

@implementation LBZXQRCodeAlignmentPatternFinder

- (id)initWithImage:(LBZXBitMatrix *)image startX:(int)startX startY:(int)startY width:(int)width height:(int)height moduleSize:(float)moduleSize resultPointCallback:(id<LBZXResultPointCallback>)resultPointCallback {
  if (self = [super init]) {
    _image = image;
    ...
    _crossCheckStateCount = [[LBZXIntArray alloc] initWithLength:3];
...
- (LBZXQRCodeAlignmentPattern *)findWithError:(NSError **)error {
  ...
  for (int iGen = 0; iGen < self.height; iGen++) {
        ...
        if (currentState == 1) {
          ...
          if (currentState == 2) {
            if ([self foundPatternCross:stateCount]) {
              LBZXQRCodeAlignmentPattern *confirmed = [self handlePossibleCenter:stateCount i:i j:j];
              if (confirmed != nil) {
                return confirmed;
              }
            }
    ...
    if ([self foundPatternCross:stateCount]) {
      LBZXQRCodeAlignmentPattern *confirmed = [self handlePossibleCenter:stateCount i:i j:maxJ];
      if (confirmed != nil) {
        return confirmed;
      }
    }
 ...
 * @return LBZXAlignmentPattern if we have found the same pattern twice, or null if not
 */
- (LBZXQRCodeAlignmentPattern *)handlePossibleCenter:(int *)stateCount i:(int)i j:(int)j {
  int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2];
  ...
  if (!isnan(centerI)) {
    ...
    for (int index = 0; index < max; index++) {
      LBZXQRCodeAlignmentPattern *center = self.possibleCenters[index];
    ...
    LBZXResultPoint *point = [[LBZXQRCodeAlignmentPattern alloc] initWithPosX:centerJ posY:centerI estimatedModuleSize:estimatedModuleSize];
