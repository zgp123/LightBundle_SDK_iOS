...
@interface LBZXQRCodeBitMatrixParser ()

@property (nonatomic, strong, readonly) LBZXBitMatrix *bitMatrix;
@property (nonatomic, assign) BOOL shouldMirror;
@property (nonatomic, strong) LBZXQRCodeFormatInformation *parsedFormatInfo;
@property (nonatomic, strong) LBZXQRCodeVersion *parsedVersion;

@end

@implementation LBZXQRCodeBitMatrixParser
...
- (LBZXQRCodeFormatInformation *)readFormatInformationWithError:(NSError **)error {
  ...
  self.parsedFormatInfo = [LBZXQRCodeFormatInformation decodeFormatInformation:formatInfoBits1 maskedFormatInfo2:formatInfoBits2];
  if (self.parsedFormatInfo != nil) {
    return self.parsedFormatInfo;
  }
  if (error) *error = LBZXFormatErrorInstance();
...
- (LBZXQRCodeVersion *)readVersionWithError:(NSError **)error {
  ...
  if (provisionalVersion <= 6) {
    return [LBZXQRCodeVersion versionForNumber:provisionalVersion];
  }
  int versionBits = 0;
  ...
  LBZXQRCodeVersion *theParsedVersion = [LBZXQRCodeVersion decodeVersionInformation:versionBits];
  ...
  theParsedVersion = [LBZXQRCodeVersion decodeVersionInformation:versionBits];
  if (theParsedVersion != nil && theParsedVersion.dimensionForVersion == dimension) {
    self.parsedVersion = theParsedVersion;
    return self.parsedVersion;
  }
...
- (LBZXByteArray *)readCodewordsWithError:(NSError **)error {
  LBZXQRCodeFormatInformation *formatInfo = [self readFormatInformationWithError:error];
  if (!formatInfo) {
    return nil;
  }

  LBZXQRCodeVersion *version = [self readVersionWithError:error];
  if (!version) {
    return nil;
  }
  ...
  LBZXQRCodeDataMask *dataMask = [LBZXQRCodeDataMask forReference:[formatInfo dataMask]];
  int dimension = self.bitMatrix.height;
  [dataMask unmaskBitMatrix:self.bitMatrix dimension:dimension];

  LBZXBitMatrix *functionPattern = [version buildFunctionPattern];

  BOOL readingUp = YES;
  LBZXByteArray *result = [[LBZXByteArray alloc] initWithLength:version.totalCodewords];
  int resultOffset = 0;
  int currentByte = 0;
  int bitsRead = 0;
...
- (void)remask {
  ...
  LBZXQRCodeDataMask *dataMask = [LBZXQRCodeDataMask forReference:self.parsedFormatInfo.dataMask];
  int dimension = self.bitMatrix.height;
  [dataMask unmaskBitMatrix:self.bitMatrix dimension:dimension];
}
