 ...
 */
const unichar LBZX_ALPHANUMERIC_CHARS[45] = {
...
const int LBZX_GB2312_SUBSET = 1;

@implementation LBZXQRCodeDecodedBitStreamParser

+ (LBZXDecoderResult *)decode:(LBZXByteArray *)bytes
                    version:(LBZXQRCodeVersion *)version
                    ecLevel:(LBZXQRCodeErrorCorrectionLevel *)ecLevel
                      hints:(LBZXDecodeHints *)hints
                      error:(NSError **)error {
  LBZXBitSource *bits = [[LBZXBitSource alloc] initWithBytes:bytes];
  ...
  LBZXCharacterSetECI *currentCharacterSetECI = nil;
  LBZXQRCodeMode *mode;
  BOOL fc1InEffect = NO;
  ...
  do {
    ...
    if ([bits available] < 4) {
      ...
      mode = [LBZXQRCodeMode terminatorMode];
    } else {
      mode = [LBZXQRCodeMode forBits:[bits readBits:4]]; // mode is encoded by 4 bits
      if (!mode) {
        if (error) *error = LBZXFormatErrorInstance();
        return nil;
      }
    }
    if (![mode isEqual:[LBZXQRCodeMode terminatorMode]]) {
      if ([mode isEqual:[LBZXQRCodeMode fnc1FirstPositionMode]] || [mode isEqual:[LBZXQRCodeMode fnc1SecondPositionMode]]) {
        // We do little with FNC1 except alter the parsed result a bit according to the spec
        fc1InEffect = YES;
      } else if ([mode isEqual:[LBZXQRCodeMode structuredAppendMode]]) {
      ...
      } else if ([mode isEqual:[LBZXQRCodeMode eciMode]]) {
        // Count doesn't apply to ECI
        int value = [self parseECIValue:bits];
        currentCharacterSetECI = [LBZXCharacterSetECI characterSetECIByValue:value];
  ...
  } while (![mode isEqual:[LBZXQRCodeMode terminatorMode]]);
 ...
 */
+ (BOOL)decodeHanziSegment:(LBZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
...
+ (BOOL)decodeKanjiSegment:(LBZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
  if (count * 13 > bits.available) {
    return NO;
  }
...
+ (BOOL)decodeByteSegment:(LBZXBitSource *)bits result:(NSMutableString *)result count:(int)count currentCharacterSetECI:(LBZXCharacterSetECI *)currentCharacterSetECI byteSegments:(NSMutableArray *)byteSegments hints:(LBZXDecodeHints *)hints {
  if (count << 3 > bits.available) {
    return NO;
  }

  LBZXByteArray *readBytes = [[LBZXByteArray alloc] initWithLength:count];
  for (int i = 0; i < count; i++) {
    readBytes.array[i] = (int8_t)[bits readBits:8];
  }
  NSStringEncoding encoding;
  if (currentCharacterSetECI == nil) {
    encoding = [LBZXStringUtils guessEncoding:readBytes hints:hints];
  } else {
...
+ (unichar)toAlphaNumericChar:(int)value {
  ...
  return LBZX_ALPHANUMERIC_CHARS[value];
}

+ (BOOL)decodeAlphanumericSegment:(LBZXBitSource *)bits result:(NSMutableString *)result count:(int)count fc1InEffect:(BOOL)fc1InEffect {
...
+ (BOOL)decodeNumericSegment:(LBZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
...
+ (int)parseECIValue:(LBZXBitSource *)bits {
