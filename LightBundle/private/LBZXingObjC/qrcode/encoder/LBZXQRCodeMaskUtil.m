...
const int LBZX_N1 = 3;
const int LBZX_N2 = 3;
const int LBZX_N3 = 40;
const int LBZX_N4 = 10;

@implementation LBZXQRCodeMaskUtil

+ (int)applyMaskPenaltyRule1:(LBZXByteMatrix *)matrix {
...
+ (int)applyMaskPenaltyRule2:(LBZXByteMatrix *)matrix {
  int penalty = 0;
  ...
  return LBZX_N2 * penalty;
}

+ (int)applyMaskPenaltyRule3:(LBZXByteMatrix *)matrix {
  int numPenalties = 0;
  int8_t **array = matrix.array;
  int width = matrix.width;
  int height = matrix.height;
...
+ (int)applyMaskPenaltyRule4:(LBZXByteMatrix *)matrix {
  int numDarkCells = 0;
  int8_t **array = matrix.array;
  int width = matrix.width;
  int height = matrix.height;
  ...
  return fivePercentVariances * LBZX_N4;
}
 ...
 */
+ (int)applyMaskPenaltyRule1Internal:(LBZXByteMatrix *)matrix isHorizontal:(BOOL)isHorizontal {
  int penalty = 0;
    ...
    for (int j = 0; j < jLimit; j++) {
      ...
      if (bit == prevBit) {
        ...
        if (numSameBitCells >= 5) {
          penalty += LBZX_N1 + (numSameBitCells - 5);
        }
