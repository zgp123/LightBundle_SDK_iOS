...
const int LBZX_QUIET_ZONE_SIZE = 4;

@implementation LBZXQRCodeWriter
...
- (LBZXBitMatrix *)encode:(NSString *)contents format:(LBZXBarcodeFormat)format width:(int)width height:(int)height hints:(LBZXEncodeHints *)hints error:(NSError **)error {
  ...
  LBZXQRCodeErrorCorrectionLevel *errorCorrectionLevel = [LBZXQRCodeErrorCorrectionLevel errorCorrectionLevelL];
  int quietZone = LBZX_QUIET_ZONE_SIZE;
  if (hints != nil) {
  ...
  LBZXQRCode *code = [LBZXQRCodeEncoder encode:contents ecLevel:errorCorrectionLevel hints:hints error:error];
  return [self renderResult:code width:width height:height quietZone:quietZone];
}

- (LBZXBitMatrix *)renderResult:(LBZXQRCode *)code width:(int)width height:(int)height quietZone:(int)quietZone {
  LBZXByteMatrix *input = code.matrix;
  if (input == nil) {
    return nil;
  }
