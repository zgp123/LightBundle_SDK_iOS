...
const int LBTHUMBNAIL_SCALE_FACTOR = 2;

@interface LBZXPlanarYUVLuminanceSource ()

@property (nonatomic, strong, readonly) LBZXByteArray *yuvData;
...
@implementation LBZXPlanarYUVLuminanceSource
...
- (id)initWithYuvData:(int8_t *)yuvData yuvDataLen:(int)yuvDataLen dataWidth:(int)dataWidth
  ...
  if (self = [super initWithWidth:width height:height]) {
    ...
    _yuvData = [[LBZXByteArray alloc] initWithLength:yuvDataLen];
...
- (LBZXByteArray *)rowAtY:(int)y row:(LBZXByteArray *)row {
  ...
  if (!row || row.length < width) {
    row = [[LBZXByteArray alloc] initWithLength:width];
  }
...
- (LBZXByteArray *)matrix {
  ...
  LBZXByteArray *matrix = [[LBZXByteArray alloc] initWithLength:area];
  ...
  LBZXByteArray *yuvData = self.yuvData;
...
- (int *)renderThumbnail {
  int thumbWidth = self.width / LBTHUMBNAIL_SCALE_FACTOR;
  int thumbHeight = self.height / LBTHUMBNAIL_SCALE_FACTOR;
  ...
  for (int y = 0; y < self.height; y++) {
    ...
    for (int x = 0; x < self.width; x++) {
      int grey = self.yuvData.array[inputOffset + x * LBTHUMBNAIL_SCALE_FACTOR] & 0xff;
      pixels[outputOffset + x] = 0xFF000000 | (grey * 0x00010101);
    }
    inputOffset += self.dataWidth * LBTHUMBNAIL_SCALE_FACTOR;
  }
  return pixels;
}

- (int)thumbnailWidth {
  return self.width / LBTHUMBNAIL_SCALE_FACTOR;
}

- (int)thumbnailHeight {
  return self.height / LBTHUMBNAIL_SCALE_FACTOR;
}
