 ...
 */
@interface LBZXDecodeHints : NSObject <NSCopying>
...
/**
 * The caller needs to be notified via callback when a possible LBZXResultPoint
 * is found.
 */
@property (nonatomic, strong) id <LBZXResultPointCallback> resultPointCallback;
 ...
 * Maps to an LBZXIntArray of the allowed extension lengths, for example [2], [5], or [2, 5].
 * If it is optional to have an extension, do not set this hint. If this is set,
 * and a UPC or EAN barcode is found but an extension is not, then no result will be returned
 * at all.
 */
@property (nonatomic, strong) LBZXIntArray *allowedEANExtensions;

/**
 * Image is known to be of one of a few possible formats.
 */
- (void)addPossibleFormat:(LBZXBarcodeFormat)format;
- (BOOL)containsFormat:(LBZXBarcodeFormat)format;
- (int)numberOfPossibleFormats;
- (void)removePossibleFormat:(LBZXBarcodeFormat)format;
