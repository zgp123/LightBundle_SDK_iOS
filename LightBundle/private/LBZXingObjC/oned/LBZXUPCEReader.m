 ...
 */
const int LBZX_UCPE_MIDDLE_END_PATTERN[] = {1, 1, 1, 1, 1, 1};

/**
 * See LBZX_UCPE_L_AND_G_PATTERNS; these values similarly represent patterns of
 * even-odd parity encodings of digits that imply both the number system (0 or 1)
 * used, and the check digit.
 */
const int LBZX_UCPE_NUMSYS_AND_CHECK_DIGIT_PATTERNS[][10] = {
  {0x38, 0x34, 0x32, 0x31, 0x2C, 0x26, 0x23, 0x2A, 0x29, 0x25},
  {0x07, 0x0B, 0x0D, 0x0E, 0x13, 0x19, 0x1C, 0x15, 0x16, 0x1A}
};

@interface LBZXUPCEReader ()

@property (nonatomic, strong, readonly) LBZXIntArray *decodeMiddleCounters;

@end

@implementation LBZXUPCEReader
...
- (NSRange)decodeEnd:(LBZXBitArray *)row endStart:(int)endStart error:(NSError **)error {
  return [LBZXUPCEANReader findGuardPattern:row
                                rowOffset:endStart
                               whiteFirst:YES
                                  pattern:LBZX_UCPE_MIDDLE_END_PATTERN
                               patternLen:sizeof(LBZX_UCPE_MIDDLE_END_PATTERN) / sizeof(int)
                                    error:error];
}

- (BOOL)checkChecksum:(NSString *)s error:(NSError **)error {
  return [super checkChecksum:[LBZXUPCEReader convertUPCEtoUPCA:s] error:error];
}

- (BOOL)determineNumSysAndCheckDigit:(NSMutableString *)resultString lgPatternFound:(int)lgPatternFound {
  for (int numSys = 0; numSys <= 1; numSys++) {
    for (int d = 0; d < 10; d++) {
      if (lgPatternFound == LBZX_UCPE_NUMSYS_AND_CHECK_DIGIT_PATTERNS[numSys][d]) {
