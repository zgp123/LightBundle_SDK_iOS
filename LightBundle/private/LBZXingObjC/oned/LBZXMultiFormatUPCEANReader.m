...
@interface LBZXMultiFormatUPCEANReader ()

@property (nonatomic, strong, readonly) NSMutableArray *readers;

@end

@implementation LBZXMultiFormatUPCEANReader
  ...
  if (self = [super init]) {
    ...
    if (hints != nil) {
      if ([hints containsFormat:kBarcodeFormatEan13]) {
        [_readers addObject:[[LBZXEAN13Reader alloc] init]];
      } else if ([hints containsFormat:kBarcodeFormatUPCA]) {
        [_readers addObject:[[LBZXUPCAReader alloc] init]];
      }

      if ([hints containsFormat:kBarcodeFormatEan8]) {
        [_readers addObject:[[LBZXEAN8Reader alloc] init]];
      }

      if ([hints containsFormat:kBarcodeFormatUPCE]) {
        [_readers addObject:[[LBZXUPCEReader alloc] init]];
      }
    }
...
- (LBZXResult *)decodeRow:(int)rowNumber row:(LBZXBitArray *)row hints:(LBZXDecodeHints *)hints error:(NSError **)error {
  NSRange startGuardPattern = [LBZXUPCEANReader findStartGuardPattern:row error:error];
  if (startGuardPattern.location == NSNotFound) {
    return nil;
  }
  for (LBZXUPCEANReader *reader in self.readers) {
    LBZXResult *result = [reader decodeRow:rowNumber row:row startGuardRange:startGuardPattern hints:hints error:error];
    if (!result) {
      continue;
    }
    ...
    if (ean13MayBeUPCA && canReturnUPCA) {
      ...
      LBZXResult *resultUPCA = [LBZXResult resultWithText:[result.text substringFromIndex:1]
