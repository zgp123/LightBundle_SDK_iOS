...
//                0    1    1   0   0    1   == 0x19
//
const int LBZX_EAN13_FIRST_DIGIT_ENCODINGS[] = {
  0x00, 0x0B, 0x0D, 0xE, 0x13, 0x19, 0x1C, 0x15, 0x16, 0x1A
};

@interface LBZXEAN13Reader ()

@property (nonatomic, strong, readonly) LBZXIntArray *decodeMiddleCounters;

@end

@implementation LBZXEAN13Reader
...
- (int)decodeMiddle:(LBZXBitArray *)row startRange:(NSRange)startRange result:(NSMutableString *)result error:(NSError **)error {
  ...
  NSRange middleRange = [[self class] findGuardPattern:row
                                             rowOffset:rowOffset
                                            whiteFirst:YES
                                               pattern:LBZX_UPC_EAN_MIDDLE_PATTERN
                                            patternLen:LBZX_UPC_EAN_MIDDLE_PATTERN_LEN
                                                 error:error];
  ...
  for (int x = 0; x < 6 && rowOffset < end; x++) {
    int bestMatch = [LBZXUPCEANReader decodeDigit:row
                                       counters:counters
                                      rowOffset:rowOffset
                                    patternType:LBZX_UPC_EAN_PATTERNS_L_PATTERNS
                                          error:error];
    if (bestMatch == -1) {
      return -1;
    }
...
- (BOOL)determineFirstDigit:(NSMutableString *)resultString lgPatternFound:(int)lgPatternFound {
  for (int d = 0; d < 10; d++) {
    if (lgPatternFound == LBZX_EAN13_FIRST_DIGIT_ENCODINGS[d]) {
