...
#import "LBZXRSSPair.h"
#import "LBZXRSSUtils.h"

const int LBZX_RSS14_OUTSIDE_EVEN_TOTAL_SUBSET[5] = {1,10,34,70,126};
const int LBZX_RSS14_INSIDE_ODD_TOTAL_SUBSET[4] = {4,20,48,81};
const int LBZX_RSS14_OUTSIDE_GSUM[5] = {0,161,961,2015,2715};
const int LBZX_RSS14_INSIDE_GSUM[4] = {0,336,1036,1516};
const int LBZX_RSS14_OUTSIDE_ODD_WIDEST[5] = {8,6,4,3,1};
const int LBZX_RSS14_INSIDE_ODD_WIDEST[4] = {2,4,6,8};

@interface LBZXRSS14Reader ()
...
@implementation LBZXRSS14Reader
...
- (LBZXResult *)decodeRow:(int)rowNumber row:(LBZXBitArray *)row hints:(LBZXDecodeHints *)hints error:(NSError **)error {
  LBZXRSSPair *leftPair = [self decodePair:row right:NO rowNumber:rowNumber hints:hints];
  [self addOrTally:self.possibleLeftPairs pair:leftPair];
  [row reverse];
  LBZXRSSPair *rightPair = [self decodePair:row right:YES rowNumber:rowNumber hints:hints];
  [self addOrTally:self.possibleRightPairs pair:rightPair];
  [row reverse];

  for (LBZXRSSPair *left in self.possibleLeftPairs) {
    if ([left count] > 1) {
      for (LBZXRSSPair *right in self.possibleRightPairs) {
        if ([right count] > 1) {
...
- (void)addOrTally:(NSMutableArray *)possiblePairs pair:(LBZXRSSPair *)pair {
  if (pair == nil) {
    return;
  }
  BOOL found = NO;
  for (LBZXRSSPair *other in possiblePairs) {
...
- (LBZXResult *)constructResult:(LBZXRSSPair *)leftPair rightPair:(LBZXRSSPair *)rightPair {
  ...
  return [LBZXResult resultWithText:buffer
                         rawBytes:nil
                     resultPoints:@[leftPoints[0], leftPoints[1], rightPoints[0], rightPoints[1]]
                           format:kBarcodeFormatRSS14];
}

- (BOOL)checkChecksum:(LBZXRSSPair *)leftPair rightPair:(LBZXRSSPair *)rightPair {
...
- (LBZXRSSPair *)decodePair:(LBZXBitArray *)row right:(BOOL)right rowNumber:(int)rowNumber hints:(LBZXDecodeHints *)hints {
  LBZXIntArray *startEnd = [self findFinderPattern:row rowOffset:0 rightFinderPattern:right];
  if (!startEnd) {
    return nil;
  }
  LBZXRSSFinderPattern *pattern = [self parseFoundFinderPattern:row rowNumber:rowNumber right:right startEnd:startEnd];
  if (!pattern) {
    return nil;
  }
  ...
  if (resultPointCallback != nil) {
    ...
    [resultPointCallback foundPossibleResultPoint:[[LBZXResultPoint alloc] initWithX:center y:rowNumber]];
  }
  LBZXRSSDataCharacter *outside = [self decodeDataCharacter:row pattern:pattern outsideChar:YES];
  LBZXRSSDataCharacter *inside = [self decodeDataCharacter:row pattern:pattern outsideChar:NO];
  if (!outside || !inside) {
    return nil;
  }
  return [[LBZXRSSPair alloc] initWithValue:1597 * outside.value + inside.value
                        checksumPortion:outside.checksumPortion + 4 * inside.checksumPortion
                          finderPattern:pattern];
}

- (LBZXRSSDataCharacter *)decodeDataCharacter:(LBZXBitArray *)row pattern:(LBZXRSSFinderPattern *)pattern outsideChar:(BOOL)outsideChar {
  ...
  if (outsideChar) {
    ...
    int oddWidest = LBZX_RSS14_OUTSIDE_ODD_WIDEST[group];
    int evenWidest = 9 - oddWidest;
    int vOdd = [LBZXRSSUtils rssValue:self.oddCounts maxWidth:oddWidest noNarrow:NO];
    int vEven = [LBZXRSSUtils rssValue:self.evenCounts maxWidth:evenWidest noNarrow:YES];
    int tEven = LBZX_RSS14_OUTSIDE_EVEN_TOTAL_SUBSET[group];
    int gSum = LBZX_RSS14_OUTSIDE_GSUM[group];
    return [[LBZXRSSDataCharacter alloc] initWithValue:vOdd * tEven + vEven + gSum checksumPortion:checksumPortion];
  } else {
    if ((evenSum & 0x01) != 0 || evenSum > 10 || evenSum < 4) {
      return nil;
    }
    int group = (10 - evenSum) / 2;
    int oddWidest = LBZX_RSS14_INSIDE_ODD_WIDEST[group];
    int evenWidest = 9 - oddWidest;
    ...
    int tOdd = LBZX_RSS14_INSIDE_ODD_TOTAL_SUBSET[group];
    int gSum = LBZX_RSS14_INSIDE_GSUM[group];
    return [[LBZXRSSDataCharacter alloc] initWithValue:vEven * tOdd + vOdd + gSum checksumPortion:checksumPortion];
  }
}

- (LBZXIntArray *)findFinderPattern:(LBZXBitArray *)row rowOffset:(int)rowOffset rightFinderPattern:(BOOL)rightFinderPattern {
...
- (LBZXRSSFinderPattern *)parseFoundFinderPattern:(LBZXBitArray *)row rowNumber:(int)rowNumber right:(BOOL)right startEnd:(LBZXIntArray *)startEnd {
  ...
  int value = [LBZXAbstractRSSReader parseFinderValue:counters finderPatternType:LBZX_RSS_PATTERNS_RSS14_PATTERNS];
  if (value == -1) {
    return nil;
  }
  ...
  return [[LBZXRSSFinderPattern alloc] initWithValue:value
                                           startEnd:[[LBZXIntArray alloc] initWithInts:firstElementStart, startEnd.array[1], -1]
                                              start:start
                                                end:end
                                          rowNumber:rowNumber];
}
