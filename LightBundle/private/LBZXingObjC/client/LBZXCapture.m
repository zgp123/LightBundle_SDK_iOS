...
@implementation LBZXCapture

- (LBZXCapture *)init {
  if (self = [super init]) {
    _captureDeviceIndex = -1;
    _captureQueue = dispatch_queue_create("com.LBZXing.captureQueue", NULL);
    _focusMode = AVCaptureFocusModeContinuousAutoFocus;
    _hardStop = NO;
    _hints = [LBZXDecodeHints hints];
    _lastScannedImage = NULL;
    _onScreen = NO;
    _orderInSkip = 0;
    _orderOutSkip = 0;

    if (NSClassFromString(@"LBZXMultiFormatReader")) {
      _reader = [NSClassFromString(@"LBZXMultiFormatReader") performSelector:@selector(reader)];
    }
...
- (void)setDelegate:(id<LBZXCaptureDelegate>)delegate {
  _delegate = delegate;
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  @autoreleasepool {
    ...
    CGImageRef videoFrameImage = [LBZXCGImageLuminanceSource createImageFromBuffer:videoFrame];
    ...
    LBZXCGImageLuminanceSource *source = [[LBZXCGImageLuminanceSource alloc] initWithCGImage:rotatedImage];
    ...
    if (self.binaryLayer || self.delegate) {
      LBZXHybridBinarizer *binarizer = [[LBZXHybridBinarizer alloc] initWithSource:self.invert ? [source invert] : source];
      ...
      if (self.delegate) {
        LBZXBinaryBitmap *bitmap = [[LBZXBinaryBitmap alloc] initWithBinarizer:binarizer];

        NSError *error;
        LBZXResult *result = [self.reader decode:bitmap hints:self.hints error:&error];
        if (result) {
          dispatch_async(dispatch_get_main_queue(), ^{
...
- (AVCaptureDevice *)device {
  ...
  if ([devices count] > 0) {
    ...
    if (!LBZXd && self.captureDeviceIndex != -1) {
      LBZXd = [devices objectAtIndex:self.captureDeviceIndex];
    }
  }

  if (!LBZXd) {
    LBZXd = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
  }

  self.captureDevice = LBZXd;
...
- (void)replaceInput {
  ...
  AVCaptureDevice *LBZXd = [self device];

  if (LBZXd) {
    self.input = [AVCaptureDeviceInput deviceInputWithDevice:LBZXd error:nil];
