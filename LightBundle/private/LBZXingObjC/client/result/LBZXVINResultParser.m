...
#import "LBZXVINParsedResult.h"
#import "LBZXVINResultParser.h"

static NSRegularExpression *LBZX_IOQ = nil;
static NSRegularExpression *LBZX_AZ09 = nil;

@implementation LBZXVINResultParser

+ (void)initialize {
  LBZX_IOQ = [[NSRegularExpression alloc] initWithPattern:@"[IOQ]" options:0 error:nil];
  LBZX_AZ09 = [[NSRegularExpression alloc] initWithPattern:@"[A-Z0-9]{17}" options:0 error:nil];
}

- (LBZXVINParsedResult *)parse:(LBZXResult *)result {
  if (result.barcodeFormat != kBarcodeFormatCode39) {
    return nil;
  }
  NSString *rawText = result.text;
  rawText = [[LBZX_IOQ stringByReplacingMatchesInString:rawText options:0 range:NSMakeRange(0, rawText.length) withTemplate:@""] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
  if ([LBZX_AZ09 matchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)] == 0) {
    return nil;
  }
  ...
  return [[LBZXVINParsedResult alloc] initWithVIN:rawText
