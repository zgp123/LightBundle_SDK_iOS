...
static NSRegularExpression *LBZX_GEO_URL_PATTERN = nil;

@implementation LBZXGeoResultParser

+ (void)initialize {
  LBZX_GEO_URL_PATTERN = [[NSRegularExpression alloc] initWithPattern:@"geo:([\\-0-9.]+),([\\-0-9.]+)(?:,([\\-0-9.]+))?(?:\\?(.*))?"
...
- (LBZXParsedResult *)parse:(LBZXResult *)result {
  ...
  NSArray *matches = [LBZX_GEO_URL_PATTERN matchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)];
  if (matches.count == 0) {
    return nil;
  }
  ...
  return [LBZXGeoParsedResult geoParsedResultWithLatitude:latitude longitude:longitude altitude:altitude query:query];
}
