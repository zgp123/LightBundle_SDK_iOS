...
static NSRegularExpression *LBZX_BEGIN_VCARD = nil;
static NSRegularExpression *LBZX_VCARD_LIKE_DATE = nil;
static NSRegularExpression *LBZX_CR_LF_SPACE_TAB = nil;
static NSRegularExpression *LBZX_NEWLINE_ESCAPE = nil;
static NSRegularExpression *LBZX_VCARD_ESCAPES = nil;
static NSString *LBZX_EQUALS = @"=";
static NSString *LBZX_SEMICOLON = @";";
static NSRegularExpression *LBZX_UNESCAPED_SEMICOLONS = nil;
static NSCharacterSet *LBZX_COMMA = nil;
static NSCharacterSet *LBZX_SEMICOLON_OR_COMMA = nil;

@implementation LBZXVCardResultParser

+ (void)initialize {
  LBZX_BEGIN_VCARD = [[NSRegularExpression alloc] initWithPattern:@"BEGIN:VCARD" options:NSRegularExpressionCaseInsensitive error:nil];
  LBZX_VCARD_LIKE_DATE = [[NSRegularExpression alloc] initWithPattern:@"\\d{4}-?\\d{2}-?\\d{2}" options:0 error:nil];
  LBZX_CR_LF_SPACE_TAB = [[NSRegularExpression alloc] initWithPattern:@"\r\n[ \t]" options:0 error:nil];
  LBZX_NEWLINE_ESCAPE = [[NSRegularExpression alloc] initWithPattern:@"\\\\[nN]" options:0 error:nil];
  LBZX_VCARD_ESCAPES = [[NSRegularExpression alloc] initWithPattern:@"\\\\([,;\\\\])" options:0 error:nil];
  LBZX_UNESCAPED_SEMICOLONS = [[NSRegularExpression alloc] initWithPattern:@"(?<!\\\\);+" options:0 error:nil];
  LBZX_COMMA = [NSCharacterSet characterSetWithCharactersInString:@","];
  LBZX_SEMICOLON_OR_COMMA = [NSCharacterSet characterSetWithCharactersInString:@";,"];
}
...
- (LBZXParsedResult *)parse:(LBZXResult *)result {
  ...
  if ([LBZX_BEGIN_VCARD numberOfMatchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)] == 0) {
    return nil;
  }
  ...
  NSArray *nicknames = nicknameString == nil ? nil : [nicknameString[0] componentsSeparatedByCharactersInSet:LBZX_COMMA];
  ...
  NSArray *geo = geoString == nil ? nil : [geoString[0] componentsSeparatedByCharactersInSet:LBZX_SEMICOLON_OR_COMMA];
  if (geo != nil && geo.count != 2) {
    geo = nil;
  }
  return [LBZXAddressBookParsedResult addressBookParsedResultWithNames:[self toPrimaryValues:names]
                                                           nicknames:nicknames
                                                       pronunciation:nil
...
+ (NSMutableArray *)matchVCardPrefixedField:(NSString *)prefix rawText:(NSString *)rawText trim:(BOOL)trim parseFieldDivider:(BOOL)parseFieldDivider {
  ...
  while (i < max) {
    ...
    if (metadataString != nil) {
      for (NSString *metadatum in [metadataString componentsSeparatedByString:LBZX_SEMICOLON]) {
        if (metadata == nil) {
          metadata = [NSMutableArray array];
        }
        [metadata addObject:metadatum];
        NSUInteger equals = [metadatum rangeOfString:LBZX_EQUALS].location;
    ...
    if (i == NSNotFound) {
      ...
      if (quotedPrintable) {
        ...
        if (parseFieldDivider) {
          element = [[LBZX_UNESCAPED_SEMICOLONS stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"\n"]
        ...
        element = [LBZX_CR_LF_SPACE_TAB stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@""];
        element = [LBZX_NEWLINE_ESCAPE stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"\n"];
        element = [LBZX_VCARD_ESCAPES stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"$1"];
      }
      if (metadata == nil) {
...
- (BOOL)isLikeVCardDate:(NSString *)value {
  return value == nil || [LBZX_VCARD_LIKE_DATE numberOfMatchesInString:value options:0 range:NSMakeRange(0, value.length)] > 0;
}
