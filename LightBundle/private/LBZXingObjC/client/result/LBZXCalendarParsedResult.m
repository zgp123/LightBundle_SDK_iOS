...
static NSRegularExpression *LBZX_DATE_TIME = nil;
static NSRegularExpression *LBZX_RFC2445_DURATION = nil;

const long LBZX_RFC2445_DURATION_FIELD_UNITS[] = {
...
@implementation LBZXCalendarParsedResult

+ (void)initialize {
  LBZX_DATE_TIME = [[NSRegularExpression alloc] initWithPattern:@"[0-9]{8}(T[0-9]{6}Z?)?"
                                                      options:0
                                                        error:nil];

  LBZX_RFC2445_DURATION = [[NSRegularExpression alloc] initWithPattern:@"P(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?"
                                                             options:NSRegularExpressionCaseInsensitive
                                                               error:nil];

//  LBZX_DATE_FORMAT = [[NSDateFormatter alloc] init];
//  LBZX_DATE_FORMAT.dateFormat = @"yyyyMMdd";
//
//  LBZX_DATE_TIME_FORMAT = [[NSDateFormatter alloc] init];
//  LBZX_DATE_TIME_FORMAT.dateFormat = @"yyyyMMdd'T'HHmmss";
}
...
- (NSString *)displayResult {
  ...
  [LBZXParsedResult maybeAppend:self.summary result:result];
  [LBZXParsedResult maybeAppend:[self format:self.startAllDay date:self.start] result:result];
  [LBZXParsedResult maybeAppend:[self format:self.endAllDay date:self.end] result:result];
  [LBZXParsedResult maybeAppend:self.location result:result];
  [LBZXParsedResult maybeAppend:self.organizer result:result];
  [LBZXParsedResult maybeAppendArray:self.attendees result:result];
  [LBZXParsedResult maybeAppend:self.description result:result];
  return result;
}
...
- (NSDate *)parseDate:(NSString *)when {
  NSArray *matches = [LBZX_DATE_TIME matchesInString:when options:0 range:NSMakeRange(0, when.length)];
  if (matches.count == 0) {
...
- (long)parseDurationMS:(NSString *)durationString {
  ...
  NSArray *m = [LBZX_RFC2445_DURATION matchesInString:durationString options:0 range:NSMakeRange(0, durationString.length)];
  if (m.count == 0) {
    return -1;
  }
  long durationMS = 0;
  NSTextCheckingResult *match = m[0];
  for (int i = 0; i < sizeof(LBZX_RFC2445_DURATION_FIELD_UNITS) / sizeof(long); i++) {
    if ([match rangeAtIndex:i + 1].location != NSNotFound) {
      ...
      if (fieldValue != nil) {
        durationMS += LBZX_RFC2445_DURATION_FIELD_UNITS[i] * [fieldValue intValue];
      }
    }
  }
  return durationMS;
