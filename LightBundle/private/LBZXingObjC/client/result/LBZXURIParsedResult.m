...
static NSRegularExpression *LBZX_USER_IN_HOST = nil;

@implementation LBZXURIParsedResult

+ (void)initialize {
  LBZX_USER_IN_HOST = [[NSRegularExpression alloc] initWithPattern:@":/*([^/@]+)@[^/]+" options:0 error:nil];
}
...
- (BOOL)possiblyMaliciousURI {
  return [LBZX_USER_IN_HOST numberOfMatchesInString:self.uri options:0 range:NSMakeRange(0, self.uri.length)] > 0;
}

- (NSString *)displayResult {
  NSMutableString *result = [NSMutableString stringWithCapacity:30];
  [LBZXParsedResult maybeAppend:self.title result:result];
  [LBZXParsedResult maybeAppend:self.uri result:result];
  return result;
}
...
- (BOOL)isColonFollowedByPortNumber:(NSString *)uri protocolEnd:(int)protocolEnd {
  ...
  return [LBZXResultParser isSubstringOfDigits:uri offset:start length:(int)nextSlash - start];
}
