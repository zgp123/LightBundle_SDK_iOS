...
const int LBZX_AZTEC_DEFAULT_EC_PERCENT = 33; // default minimal percentage of error check words
const int LBZX_AZTEC_DEFAULT_LAYERS = 0;
const int LBZX_AZTEC_MAX_NB_BITS = 32;
const int LBZX_AZTEC_MAX_NB_BITS_COMPACT = 4;

const int LBZX_AZTEC_WORD_SIZE[] = {
  4, 6, 6, 8, 8, 8, 8, 8, 8, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
  12, 12, 12, 12, 12, 12, 12, 12, 12, 12
};

@implementation LBZXAztecEncoder

+ (LBZXAztecCode *)encode:(LBZXByteArray *)data {
  return [self encode:data minECCPercent:LBZX_AZTEC_DEFAULT_EC_PERCENT userSpecifiedLayers:LBZX_AZTEC_DEFAULT_LAYERS];
}

+ (LBZXAztecCode *)encode:(LBZXByteArray *)data minECCPercent:(int)minECCPercent userSpecifiedLayers:(int)userSpecifiedLayers {
  // High-level encode
  LBZXBitArray *bits = [[[LBZXAztecHighLevelEncoder alloc] initWithText:data] encode];
  ...
  int wordSize;
  LBZXBitArray *stuffedBits;
  if (userSpecifiedLayers != LBZX_AZTEC_DEFAULT_LAYERS) {
    compact = userSpecifiedLayers < 0;
    layers = abs(userSpecifiedLayers);
    if (layers > (compact ? LBZX_AZTEC_MAX_NB_BITS_COMPACT : LBZX_AZTEC_MAX_NB_BITS)) {
    ...
    wordSize = LBZX_AZTEC_WORD_SIZE[layers];
    ...
    for (int i = 0; ; i++) {
      ...
      if (wordSize != LBZX_AZTEC_WORD_SIZE[layers]) {
        wordSize = LBZX_AZTEC_WORD_SIZE[layers];
        stuffedBits = [self stuffBits:bits wordSize:wordSize];
      }
      int usableBitsInLayers = totalBitsInLayer - (totalBitsInLayer % wordSize);
  ...
  LBZXBitArray *messageBits = [self generateCheckWords:stuffedBits totalBits:totalBitsInLayer wordSize:wordSize];

  // generate check words
  int messageSizeInWords = stuffedBits.size / wordSize;
  LBZXBitArray *modeMessage = [self generateModeMessageCompact:compact layers:layers messageSizeInWords:messageSizeInWords];
  ...
  LBZXBitMatrix *matrix = [[LBZXBitMatrix alloc] initWithDimension:matrixSize];
  ...
  LBZXAztecCode *aztec = [[LBZXAztecCode alloc] init];
...
+ (void)drawBullsEye:(LBZXBitMatrix *)matrix center:(int)center size:(int)size {
...
+ (LBZXBitArray *)generateModeMessageCompact:(BOOL)compact layers:(int)layers messageSizeInWords:(int)messageSizeInWords {
  LBZXBitArray *modeMessage = [[LBZXBitArray alloc] init];
  if (compact) {
...
+ (void)drawModeMessage:(LBZXBitMatrix *)matrix compact:(BOOL)compact matrixSize:(int)matrixSize modeMessage:(LBZXBitArray *)modeMessage {
  int center = matrixSize / 2;
  if (compact) {
    for (int i = 0; i < 7; i++) {
...
+ (LBZXBitArray *)generateCheckWords:(LBZXBitArray *)bitArray totalBits:(int)totalBits wordSize:(int)wordSize {
  // bitArray is guaranteed to be a multiple of the wordSize, so no padding needed
  int messageSizeInWords = bitArray.size / wordSize;
  LBZXReedSolomonEncoder *rs = [[LBZXReedSolomonEncoder alloc] initWithField:[self getGF:wordSize]];
  int totalWords = totalBits / wordSize;

  LBZXIntArray *messageWords = [self bitsToWords:bitArray wordSize:wordSize totalWords:totalWords];
  [rs encode:messageWords ecBytes:totalWords - messageSizeInWords];
  int startPad = totalBits % wordSize;
  LBZXBitArray *messageBits = [[LBZXBitArray alloc] init];
...
+ (LBZXIntArray *)bitsToWords:(LBZXBitArray *)stuffedBits wordSize:(int)wordSize totalWords:(int)totalWords {
  LBZXIntArray *message = [[LBZXIntArray alloc] initWithLength:totalWords];
  int i;
  int n;
...
+ (LBZXGenericGF *)getGF:(int)wordSize {
  switch (wordSize) {
    case 4:
      return [LBZXGenericGF AztecParam];
    case 6:
      return [LBZXGenericGF AztecData6];
    case 8:
      return [LBZXGenericGF AztecData8];
    case 10:
      return [LBZXGenericGF AztecData10];
    case 12:
      return [LBZXGenericGF AztecData12];
    default:
      return nil;
  }
}

+ (LBZXBitArray *)stuffBits:(LBZXBitArray *)bits wordSize:(int)wordSize {
  LBZXBitArray *arrayOut = [[LBZXBitArray alloc] init];
