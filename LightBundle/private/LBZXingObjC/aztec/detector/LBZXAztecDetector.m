...
- (LBZXResultPoint *)toResultPoint {
  return [[LBZXResultPoint alloc] initWithX:self.x y:self.y];
}
...
@interface LBZXAztecDetector ()

@property (nonatomic, assign, getter = isCompact) BOOL compact;
@property (nonatomic, strong) LBZXBitMatrix *image;
...
@implementation LBZXAztecDetector
...
- (LBZXAztecDetectorResult *)detectWithMirror:(BOOL)isMirror error:(NSError **)error {
  ...
  LBZXAztecPoint *pCenter = [self matrixCenter];
  if (!pCenter) {
  ...
  if (isMirror) {
    LBZXResultPoint *temp = bullsEyeCorners[0];
  ...
  // 4. Sample the grid
  LBZXBitMatrix *bits = [self sampleGrid:self.image
  ...
  return [[LBZXAztecDetectorResult alloc] initWithBits:bits
                                               points:corners
...
- (BOOL)extractParameters:(NSArray *)bullsEyeCorners {
  LBZXResultPoint *p0 = bullsEyeCorners[0];
  LBZXResultPoint *p1 = bullsEyeCorners[1];
  LBZXResultPoint *p2 = bullsEyeCorners[2];
  LBZXResultPoint *p3 = bullsEyeCorners[3];
...
- (int)rotationForSides:(const int[])sides length:(int)length {
  ...
  for (int shift = 0; shift < 4; shift++) {
    if (LBbitCount(cornerBits ^ LBexpectedCornerBits[shift]) <= 2) {
      return shift;
    }
  }
  return -1;
...
- (int)correctedParameterData:(long)parameterData compact:(BOOL)compact {
  ...
  LBZXIntArray *parameterWords = [[LBZXIntArray alloc] initWithLength:numCodewords];
  for (int i = numCodewords - 1; i >= 0; --i) {
    parameterWords.array[i] = (int32_t) parameterData & 0xF;
    parameterData >>= 4;
  }

  LBZXReedSolomonDecoder *rsDecoder = [[LBZXReedSolomonDecoder alloc] initWithField:[LBZXGenericGF AztecParam]];
 ...
 */
- (NSMutableArray *)bullsEyeCorners:(LBZXAztecPoint *)pCenter {
  LBZXAztecPoint *pina = pCenter;
  LBZXAztecPoint *pinb = pCenter;
  LBZXAztecPoint *pinc = pCenter;
  LBZXAztecPoint *pind = pCenter;

  BOOL color = YES;

  for (self.nbCenterLayers = 1; self.nbCenterLayers < 9; self.nbCenterLayers++) {
    LBZXAztecPoint *pouta = [self firstDifferent:pina color:color dx:1 dy:-1];
    LBZXAztecPoint *poutb = [self firstDifferent:pinb color:color dx:1 dy:1];
    LBZXAztecPoint *poutc = [self firstDifferent:pinc color:color dx:-1 dy:1];
    LBZXAztecPoint *poutd = [self firstDifferent:pind color:color dx:-1 dy:-1];
  ...
  LBZXResultPoint *pinax = [[LBZXResultPoint alloc] initWithX:pina.x + 0.5f y:pina.y - 0.5f];
  LBZXResultPoint *pinbx = [[LBZXResultPoint alloc] initWithX:pinb.x + 0.5f y:pinb.y + 0.5f];
  LBZXResultPoint *pincx = [[LBZXResultPoint alloc] initWithX:pinc.x - 0.5f y:pinc.y + 0.5f];
  LBZXResultPoint *pindx = [[LBZXResultPoint alloc] initWithX:pind.x - 0.5f y:pind.y - 0.5f];
 ...
 */
- (LBZXAztecPoint *)matrixCenter {
  LBZXResultPoint *pointA;
  LBZXResultPoint *pointB;
  LBZXResultPoint *pointC;
  LBZXResultPoint *pointD;

  LBZXWhiteRectangleDetector *detector = [[LBZXWhiteRectangleDetector alloc] initWithImage:self.image error:nil];
  ...
  if (cornerPoints) {
    ...
    pointA = [[self firstDifferent:[[LBZXAztecPoint alloc] initWithX:cx + 7 y:cy - 7] color:NO dx:1 dy:-1] toResultPoint];
    pointB = [[self firstDifferent:[[LBZXAztecPoint alloc] initWithX:cx + 7 y:cy + 7]  color:NO dx:1 dy:1] toResultPoint];
    pointC = [[self firstDifferent:[[LBZXAztecPoint alloc] initWithX:cx - 7 y:cy + 7]  color:NO dx:-1 dy:1] toResultPoint];
    pointD = [[self firstDifferent:[[LBZXAztecPoint alloc] initWithX:cx - 7 y:cy - 7]  color:NO dx:-1 dy:-1] toResultPoint];
  }

  //Compute the center of the rectangle
  int cx = [LBZXMathUtils round:([pointA x] + [pointD x] + [pointB x] + [pointC x]) / 4.0f];
  int cy = [LBZXMathUtils round:([pointA y] + [pointD y] + [pointB y] + [pointC y]) / 4.0f];

  // Redetermine the white rectangle starting from previously computed center.
  // This will ensure that we end up with a white rectangle in center bull's eye
  // in order to compute a more accurate center.
  detector = [[LBZXWhiteRectangleDetector alloc] initWithImage:self.image initSize:15 x:cx y:cy error:nil];
  ...
  cx = [LBZXMathUtils round:([pointA x] + [pointD x] + [pointB x] + [pointC x]) / 4];
  cy = [LBZXMathUtils round:([pointA y] + [pointD y] + [pointB y] + [pointC y]) / 4];

  // Recompute the center of the rectangle
  return [[LBZXAztecPoint alloc] initWithX:cx y:cy];
}
 ...
 */
- (LBZXBitMatrix *)sampleGrid:(LBZXBitMatrix *)anImage
 ...
 */
- (int)sampleLine:(LBZXResultPoint *)p1 p2:(LBZXResultPoint *)p2 size:(int)size {
  int result = 0;
  ...
  for (int i = 0; i < size; i++) {
    if ([self.image getX:[LBZXMathUtils round:px + i * dx] y:[LBZXMathUtils round:py + i * dy]]) {
      result |= 1 << (size - i - 1);
    }
  }
 ...
 */
- (BOOL)isWhiteOrBlackRectangle:(LBZXAztecPoint *)p1 p2:(LBZXAztecPoint *)p2 p3:(LBZXAztecPoint *)p3 p4:(LBZXAztecPoint *)p4 {
  int corr = 3;

  p1 = [[LBZXAztecPoint alloc] initWithX:p1.x - corr y:p1.y + corr];
  p2 = [[LBZXAztecPoint alloc] initWithX:p2.x - corr y:p2.y - corr];
  p3 = [[LBZXAztecPoint alloc] initWithX:p3.x + corr y:p3.y - corr];
  p4 = [[LBZXAztecPoint alloc] initWithX:p4.x + corr y:p4.y + corr];
 ...
 */
- (int)color:(LBZXAztecPoint *)p1 p2:(LBZXAztecPoint *)p2 {
  ...
  for (int i = 0; i < d; i++) {
    px += dx;
    py += dy;
    if ([self.image getX:[LBZXMathUtils round:px] y:[LBZXMathUtils round:py]] != colorModel) {
      error++;
    }
  }
 ...
 */
- (LBZXAztecPoint *)firstDifferent:(LBZXAztecPoint *)init color:(BOOL)color dx:(int)dx dy:(int)dy {
  int x = init.x + dx;
  int y = init.y + dy;
  ...
  return [[LBZXAztecPoint alloc] initWithX:x y:y];
}
...
- (NSArray *)expandSquare:(NSArray *)cornerPoints oldSide:(float)oldSide newSide:(float)newSide {
  LBZXResultPoint *cornerPoints0 = (LBZXResultPoint *)cornerPoints[0];
  LBZXResultPoint *cornerPoints1 = (LBZXResultPoint *)cornerPoints[1];
  LBZXResultPoint *cornerPoints2 = (LBZXResultPoint *)cornerPoints[2];
  LBZXResultPoint *cornerPoints3 = (LBZXResultPoint *)cornerPoints[3];
  ...
  LBZXResultPoint *result0 = [[LBZXResultPoint alloc] initWithX:centerx + ratio * dx y:centery + ratio * dy];
  LBZXResultPoint *result2 = [[LBZXResultPoint alloc] initWithX:centerx - ratio * dx y:centery - ratio * dy];
  ...
  LBZXResultPoint *result1 = [[LBZXResultPoint alloc] initWithX:centerx + ratio * dx y:centery + ratio * dy];
  LBZXResultPoint *result3 = [[LBZXResultPoint alloc] initWithX:centerx - ratio * dx y:centery - ratio * dy];
...
- (BOOL)isValid:(LBZXResultPoint *)point {
  int x = [LBZXMathUtils round:point.x];
  int y = [LBZXMathUtils round:point.y];
  return [self isValidX:x y:y];
}

- (float)distance:(LBZXAztecPoint *)a b:(LBZXAztecPoint *)b {
  return [LBZXMathUtils distance:a.x aY:a.y bX:b.x bY:b.y];
}

- (float)resultDistance:(LBZXResultPoint *)a b:(LBZXResultPoint *)b {
  return [LBZXMathUtils distance:a.x aY:a.y bX:b.x bY:b.y];
}
