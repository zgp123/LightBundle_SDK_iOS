...
#import "LBZXDataMatrixDefaultPlacement.h"
#import "LBZXDataMatrixErrorCorrection.h"
...
@implementation LBZXDataMatrixWriter
...
- (LBZXBitMatrix *)encode:(NSString *)contents format:(LBZXBarcodeFormat)format width:(int)width height:(int)height hints:(LBZXEncodeHints *)hints error:(NSError **)error {
  ...
  LBZXDataMatrixSymbolShapeHint shape = LBZXDataMatrixSymbolShapeHintForceNone;
  LBZXDimension *minSize = nil;
  LBZXDimension *maxSize = nil;
  if (hints != nil) {
    shape = hints.dataMatrixShape;
    LBZXDimension *requestedMinSize = hints.minSize;
    if (requestedMinSize != nil) {
      minSize = requestedMinSize;
    }
    LBZXDimension *requestedMaxSize = hints.maxSize;
  ...
  NSString *encoded = [LBZXDataMatrixHighLevelEncoder encodeHighLevel:contents shape:shape minSize:minSize maxSize:maxSize];

  LBZXDataMatrixSymbolInfo *symbolInfo = [LBZXDataMatrixSymbolInfo lookup:(int)encoded.length shape:shape minSize:minSize maxSize:maxSize fail:YES];

  //2. step: ECC generation
  NSString *codewords = [LBZXDataMatrixErrorCorrection encodeECC200:encoded symbolInfo:symbolInfo];

  //3. step: Module placement in Matrix
  LBZXDataMatrixDefaultPlacement *placement = [[LBZXDataMatrixDefaultPlacement alloc] initWithCodewords:codewords numcols:symbolInfo.symbolDataWidth numrows:symbolInfo.symbolDataHeight];
  [placement place];
 ...
 */
- (LBZXBitMatrix *)encodeLowLevel:(LBZXDataMatrixDefaultPlacement *)placement symbolInfo:(LBZXDataMatrixSymbolInfo *)symbolInfo width:(int)width height:(int)height {
  int symbolWidth = symbolInfo.symbolDataWidth;
  int symbolHeight = symbolInfo.symbolDataHeight;

  LBZXByteMatrix *matrix = [[LBZXByteMatrix alloc] initWithWidth:symbolInfo.symbolWidth height:symbolInfo.symbolHeight];
...
/**
 * Convert the LBZXByteMatrix to LBZXBitMatrix.
 *
 * @param matrix The input matrix.
 * @return The output matrix.
 */
- (LBZXBitMatrix *)convertByteMatrixToBitMatrix:(LBZXByteMatrix *)input width:(int)width height:(int)height {
