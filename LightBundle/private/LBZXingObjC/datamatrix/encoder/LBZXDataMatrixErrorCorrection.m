 ...
 * See FACTORS.
 */
const int LBZX_FACTOR_SETS[] = {5, 7, 10, 11, 12, 14, 18, 20, 24, 28, 36, 42, 48, 56, 62, 68};

/**
 * Precomputed polynomial factors for ECC 200.
 */
const int LBZX_FACTORS[16][68] = {
  {228, 48, 15, 111, 62},
...
const int LBZX_MODULO_VALUE = 0x12D;

static int LBZX_LOG[256], LBZX_ALOG[256];

@implementation LBZXDataMatrixErrorCorrection
...
+ (NSString *)createECCBlock:(NSString *)codewords start:(int)start len:(int)len numECWords:(int)numECWords {
  int table = -1;
  for (int i = 0; i < sizeof(LBZX_FACTOR_SETS) / sizeof(int); i++) {
    if (LBZX_FACTOR_SETS[i] == numECWords) {
      table = i;
      break;
    }
  }
  ...
  int *poly = (int *)LBZX_FACTORS[table];
  ...
  for (int i = start; i < start + len; i++) {
    ...
    for (int k = numECWords - 1; k > 0; k--) {
      if (m != 0 && poly[k] != 0) {
        ecc[k] = (unichar) (ecc[k - 1] ^ LBZX_ALOG[(LBZX_LOG[m] + LBZX_LOG[poly[k]]) % 255]);
      } else {
        ecc[k] = ecc[k - 1];
      }
    }
    if (m != 0 && poly[0] != 0) {
      ecc[0] = (unichar) LBZX_ALOG[(LBZX_LOG[m] + LBZX_LOG[poly[0]]) % 255];
    } else {
      ecc[0] = 0;
    }
  }
