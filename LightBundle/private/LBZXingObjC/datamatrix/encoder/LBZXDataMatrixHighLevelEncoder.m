...
#import "LBZXDataMatrixBase256Encoder.h"
#import "LBZXDataMatrixC40Encoder.h"
#import "LBZXDataMatrixEdifactEncoder.h"
#import "LBZXDataMatrixEncoderContext.h"
#import "LBZXDataMatrixHighLevelEncoder.h"
#import "LBZXDataMatrixSymbolInfo.h"
#import "LBZXDataMatrixTextEncoder.h"
#import "LBZXDataMatrixX12Encoder.h"

/**
 * Padding character
 */
const unichar LBPAD_CHAR = 129;
...
@implementation LBZXDataMatrixHighLevelEncoder
...
+ (NSString *)encodeHighLevel:(NSString *)msg {
  return [self encodeHighLevel:msg shape:LBZXDataMatrixSymbolShapeHintForceNone minSize:nil maxSize:nil];
}

+ (NSString *)encodeHighLevel:(NSString *)msg shape:(LBZXDataMatrixSymbolShapeHint)shape
                      minSize:(LBZXDimension *)minSize maxSize:(LBZXDimension *)maxSize {
  //the codewords 0..255 are encoded as Unicode characters
  NSArray *encoders = @[[[LBZXDataMatrixASCIIEncoder alloc] init],
                        [[LBZXDataMatrixC40Encoder alloc] init],
                        [[LBZXDataMatrixTextEncoder alloc] init],
                        [[LBZXDataMatrixX12Encoder alloc] init],
                        [[LBZXDataMatrixEdifactEncoder alloc] init],
                        [[LBZXDataMatrixBase256Encoder alloc] init]];

  LBZXDataMatrixEncoderContext *context = [[LBZXDataMatrixEncoderContext alloc] initWithMessage:msg];
  ...
  if (codewords.length < capacity) {
    [codewords appendFormat:@"%C", LBPAD_CHAR];
  }
  while (codewords.length < capacity) {
    [codewords appendFormat:@"%C", [self randomize253State:LBPAD_CHAR codewordPosition:(int)codewords.length + 1]];
  }
