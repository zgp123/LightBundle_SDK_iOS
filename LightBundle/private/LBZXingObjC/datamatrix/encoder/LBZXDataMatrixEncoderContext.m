...
@interface LBZXDataMatrixEncoderContext ()
...
@implementation LBZXDataMatrixEncoderContext
...
- (id)initWithMessage:(NSString *)msg {
  if (self = [super init]) {
    ...
    _symbolShape = LBZXDataMatrixSymbolShapeHintForceNone;
...
- (void)updateSymbolInfoWithLength:(int)len {
  if (self.symbolInfo == nil || len > self.symbolInfo.dataCapacity) {
    self.symbolInfo = [LBZXDataMatrixSymbolInfo lookup:len shape:self.symbolShape minSize:self.minSize maxSize:self.maxSize fail:YES];
  }
}
