...
@interface LBZXDataMatrixBitMatrixParser ()

@property (nonatomic, strong, readonly) LBZXBitMatrix *mappingBitMatrix;
@property (nonatomic, strong, readonly) LBZXBitMatrix *readMappingMatrix;

@end

@implementation LBZXDataMatrixBitMatrixParser
...
- (id)initWithBitMatrix:(LBZXBitMatrix *)bitMatrix error:(NSError **)error {
  if (self = [super init]) {
    ...
    _readMappingMatrix = [[LBZXBitMatrix alloc] initWithWidth:_mappingBitMatrix.width
 ...
 *
 * @param bitMatrix Original LBZXBitMatrix including alignment patterns
 * @return LBZXDatamatrixVersion encapsulating the Data Matrix Code's "version"
 *  or nil if the dimensions of the mapping matrix are not valid
 *  Data Matrix dimensions.
 */
- (LBZXDataMatrixVersion *)readVersion:(LBZXBitMatrix *)bitMatrix {
  int numRows = bitMatrix.height;
  int numColumns = bitMatrix.width;
  return [LBZXDataMatrixVersion versionForDimensions:numRows numColumns:numColumns];
}

- (LBZXByteArray *)readCodewords {
  LBZXByteArray *result = [[LBZXByteArray alloc] initWithLength:self.version.totalCodewords];
  int resultOffset = 0;
...
/**
 * Extracts the data region from a LBZXBitMatrix that contains
 * alignment patterns.
 *
 * @param bitMatrix Original LBZXBitMatrix with alignment patterns
 * @return BitMatrix that has the alignment patterns removed
 */
- (LBZXBitMatrix *)extractDataRegion:(LBZXBitMatrix *)bitMatrix {
  ...
  LBZXBitMatrix *bitMatrixWithoutAlignment = [[LBZXBitMatrix alloc] initWithWidth:sizeDataRegionColumn height:sizeDataRegionRow];
