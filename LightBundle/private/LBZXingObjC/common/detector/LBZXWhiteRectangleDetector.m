...
@interface LBZXWhiteRectangleDetector ()
...
const int LBZX_INIT_SIZE = 10;
const int LBZX_CORR = 1;

@implementation LBZXWhiteRectangleDetector

- (id)initWithImage:(LBZXBitMatrix *)image error:(NSError **)error {
  return [self initWithImage:image initSize:LBZX_INIT_SIZE x:image.width / 2 y:image.height / 2 error:error];
}

- (id)initWithImage:(LBZXBitMatrix *)image initSize:(int)initSize x:(int)x y:(int)y error:(NSError **)error {
  if (self = [super init]) {
    _image = image;
...
- (LBZXResultPoint *)blackPointOnSegment:(float)aX aY:(float)aY bX:(float)bX bY:(float)bY {
  int dist = [LBZXMathUtils round:[LBZXMathUtils distance:aX aY:aY bX:bX bY:bY]];
  float xStep = (bX - aX) / dist;
  float yStep = (bY - aY) / dist;

  for (int i = 0; i < dist; i++) {
    int x = [LBZXMathUtils round:aX + i * xStep];
    int y = [LBZXMathUtils round:aY + i * yStep];
    if ([self.image getX:x y:y]) {
      return [[LBZXResultPoint alloc] initWithX:x y:y];
    }
  }
 ...
 * @return LBZXResultPoint array describing the corners of the rectangular
 *         region. The first and last points are opposed on the diagonal, as
 *         are the second and third. The first point will be the topmost
 *         point and the last, the bottommost. The second point will be
 *         leftmost and the third, the rightmost
 */
- (NSArray *)centerEdges:(LBZXResultPoint *)y z:(LBZXResultPoint *)z x:(LBZXResultPoint *)x t:(LBZXResultPoint *)t {
  //
  //       t            t
  //  z                      x
  //        x    OR    z
  ...
  if (yi < self.width / 2.0f) {
    return @[[[LBZXResultPoint alloc] initWithX:ti - LBZX_CORR y:tj + LBZX_CORR],
             [[LBZXResultPoint alloc] initWithX:zi + LBZX_CORR y:zj + LBZX_CORR],
             [[LBZXResultPoint alloc] initWithX:xi - LBZX_CORR y:xj - LBZX_CORR],
             [[LBZXResultPoint alloc] initWithX:yi + LBZX_CORR y:yj - LBZX_CORR]];
  } else {
    return @[[[LBZXResultPoint alloc] initWithX:ti + LBZX_CORR y:tj + LBZX_CORR],
             [[LBZXResultPoint alloc] initWithX:zi + LBZX_CORR y:zj - LBZX_CORR],
             [[LBZXResultPoint alloc] initWithX:xi - LBZX_CORR y:xj + LBZX_CORR],
