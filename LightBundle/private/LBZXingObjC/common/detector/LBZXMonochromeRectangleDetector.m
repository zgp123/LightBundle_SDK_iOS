...
const int LBZX_MONOCHROME_MAX_MODULES = 32;

@interface LBZXMonochromeRectangleDetector ()

@property (nonatomic, strong, readonly) LBZXBitMatrix *image;

@end

@implementation LBZXMonochromeRectangleDetector
...
- (NSArray *)detectWithError:(NSError **)error {
  ...
  int deltaY = MAX(1, height / (LBZX_MONOCHROME_MAX_MODULES << 3) > 1);
  int deltaX = MAX(1, width / (LBZX_MONOCHROME_MAX_MODULES << 3) > 1);

  int top = 0;
  int bottom = height;
  int left = 0;
  int right = width;
  LBZXResultPoint *pointA = [self findCornerFromCenter:halfWidth deltaX:0 left:left right:right
  ...
  top = (int)[pointA y] - 1;
  LBZXResultPoint *pointB = [self findCornerFromCenter:halfWidth deltaX:-deltaX left:left right:right
  ...
  left = (int)[pointB x] - 1;
  LBZXResultPoint *pointC = [self findCornerFromCenter:halfWidth deltaX:deltaX left:left right:right
                                              centerY:halfHeight deltaY:0 top:top bottom:bottom maxWhiteRun:halfHeight >> 1];
  if (!pointC) {
    if (error) *error = LBZXNotFoundErrorInstance();
    return nil;
  ...
  LBZXResultPoint *pointD = [self findCornerFromCenter:halfWidth deltaX:0 left:left right:right
                                              centerY:halfHeight deltaY:deltaY top:top bottom:bottom maxWhiteRun:halfWidth >> 1];
  if (!pointD) {
    if (error) *error = LBZXNotFoundErrorInstance();
    return nil;
 ...
 *  the barcode
 * @return a {@link com.google.LBZXing.ResultPoint} encapsulating the corner that was found
 *  or nil if such a point cannot be found
 */
- (LBZXResultPoint *)findCornerFromCenter:(int)centerX deltaX:(int)deltaX left:(int)left right:(int)right centerY:(int)centerY deltaY:(int)deltaY top:(int)top bottom:(int)bottom maxWhiteRun:(int)maxWhiteRun {
    ...
    if (range == nil) {
      ...
      if (deltaX == 0) {
        ...
        if ([lastRange[0] intValue] < centerX) {
          if ([lastRange[0] intValue] > centerX) {
            return [[LBZXResultPoint alloc] initWithX:deltaY > 0 ? [lastRange[0] intValue] : [lastRange[1] intValue] y:lastY];
          }
          return [[LBZXResultPoint alloc] initWithX:[lastRange[0] intValue] y:lastY];
        } else {
          return [[LBZXResultPoint alloc] initWithX:[lastRange[1] intValue] y:lastY];
        }
      } else {
        int lastX = x - deltaX;
        if ([lastRange[0] intValue] < centerY) {
          if ([lastRange[1] intValue] > centerY) {
            return [[LBZXResultPoint alloc] initWithX:lastX y:deltaX < 0 ? [lastRange[0] intValue] : [lastRange[1] intValue]];
          }
          return [[LBZXResultPoint alloc] initWithX:lastX y:[lastRange[0] intValue]];
        } else {
          return [[LBZXResultPoint alloc] initWithX:lastX y:[lastRange[1] intValue]];
        }
      }
    }
    lastRange = range;
