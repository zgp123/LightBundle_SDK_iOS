...
@implementation LBZXDefaultGridSampler
...
- (LBZXBitMatrix *)sampleGrid:(LBZXBitMatrix *)image
                 ...
                 dimensionY:(int)dimensionY
                    ...
                    p4FromX:(float)p4FromX p4FromY:(float)p4FromY
                      error:(NSError **)error {
  LBZXPerspectiveTransform *transform =
    [LBZXPerspectiveTransform quadrilateralToQuadrilateral:p1ToX y0:p1ToY
                                                      x1:p2ToX y1:p2ToY
                                                      x2:p3ToX y2:p3ToY
                                                      x3:p4ToX y3:p4ToY
...
- (LBZXBitMatrix *)sampleGrid:(LBZXBitMatrix *)image
  ...
  LBZXBitMatrix *bits = [[LBZXBitMatrix alloc] initWithWidth:dimensionX height:dimensionY];
  ...
  for (int y = 0; y < dimensionY; y++) {
    ...
    if (![LBZXGridSampler checkAndNudgePoints:image points:pointsf pointsLen:pointsLen error:error]) {
      return nil;
    }
    for (int x = 0; x < max; x += 2) {
