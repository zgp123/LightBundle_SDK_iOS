...
@implementation LBZXCharacterSetECI
...
+ (void)addCharacterSet:(int)value encoding:(NSStringEncoding)encoding {
  LBZXCharacterSetECI *eci = [[LBZXCharacterSetECI alloc] initWithValue:value encoding:encoding];
  VALUE_TO_ECI[@(value)] = eci;
  ENCODING_TO_ECI[@(encoding)] = eci;
}

+ (LBZXCharacterSetECI *)characterSetECIByValue:(int)value {
  if (VALUE_TO_ECI == nil) {
    [self initialize];
  }
...
+ (LBZXCharacterSetECI *)characterSetECIByEncoding:(NSStringEncoding)encoding {
