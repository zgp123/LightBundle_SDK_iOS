...
- (LBZXBitArray *)rowAtY:(int)y row:(LBZXBitArray *)row {
  if (row == nil || [row size] < self.width) {
    row = [[LBZXBitArray alloc] initWithSize:self.width];
  } else {
    [row clear];
  }
...
- (void)rotate180 {
  ...
  LBZXBitArray *topRow = [[LBZXBitArray alloc] initWithSize:width];
  LBZXBitArray *bottomRow = [[LBZXBitArray alloc] initWithSize:width];
...
- (LBZXIntArray *)enclosingRectangle {
  ...
  return [[LBZXIntArray alloc] initWithInts:left, top, width, height, -1];
}
...
- (LBZXIntArray *)topLeftOnBit {
  ...
  x += bit;
  return [[LBZXIntArray alloc] initWithInts:x, y, -1];
}
...
- (BOOL)isEqual:(NSObject *)o {
  if (!([o isKindOfClass:[LBZXBitMatrix class]])) {
    return NO;
  }
  LBZXBitMatrix *other = (LBZXBitMatrix *)o;
...
- (id)copyWithZone:(NSZone *)zone {
  return [[LBZXBitMatrix allocWithZone:zone] initWithWidth:self.width height:self.height rowSize:self.rowSize bits:self.bits];
}
