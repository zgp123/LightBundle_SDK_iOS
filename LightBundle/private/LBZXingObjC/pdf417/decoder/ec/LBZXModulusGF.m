...
+ (LBZXModulusGF *)PDF417_GF {
  ...
  dispatch_once(&pred, ^{
    @autoreleasepool {
      _mod = [[LBZXModulusGF alloc] initWithModulus:LBZX_PDF417_NUMBER_OF_CODEWORDS generator:3];
    }
  });
  return _mod;
}
...
- (id)initWithModulus:(int)modulus generator:(int)generator {
  if (self = [super init]) {
    ...
    _zero = [[LBZXModulusPoly alloc] initWithField:self coefficients:[[LBZXIntArray alloc] initWithLength:1]];
    _one = [[LBZXModulusPoly alloc] initWithField:self coefficients:[[LBZXIntArray alloc] initWithInts:1, -1]];
  }
...
- (LBZXModulusPoly *)buildMonomial:(int)degree coefficient:(int)coefficient {
  ...
  return [[LBZXModulusPoly alloc] initWithField:self coefficients:coefficients];
}
