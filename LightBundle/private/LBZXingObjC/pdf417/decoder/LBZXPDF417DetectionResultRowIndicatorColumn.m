...
@implementation LBZXPDF417DetectionResultRowIndicatorColumn

- (id)initWithBoundingBox:(LBZXPDF417BoundingBox *)boundingBox isLeft:(BOOL)isLeft {
...
- (int)adjustCompleteIndicatorColumnRowNumbers:(LBZXPDF417BarcodeMetadata *)barcodeMetadata {
  [self setRowNumbers];
  [self removeIncorrectCodewords:barcodeMetadata];
  LBZXResultPoint *top = self.isLeft ? self.boundingBox.topLeft : self.boundingBox.topRight;
  LBZXResultPoint *bottom = self.isLeft ? self.boundingBox.bottomLeft : self.boundingBox.bottomRight;
  ...
  for (int codewordsRow = firstRow; codewordsRow < lastRow; codewordsRow++) {
    ...
    LBZXPDF417Codeword *codeword = self.codewords[codewordsRow];
...
- (LBZXIntArray *)rowHeights {
  LBZXPDF417BarcodeMetadata *barcodeMetadata = [self barcodeMetadata];
  if (!barcodeMetadata) {
    return nil;
  }
  [self adjustIncompleteIndicatorColumnRowNumbers:barcodeMetadata];
  LBZXIntArray *result = [[LBZXIntArray alloc] initWithLength:barcodeMetadata.rowCount];
  for (LBZXPDF417Codeword *codeword in [self codewords]) {
    if ((id)codeword != [NSNull null]) {
...
// finding row numbers for other columns easier
// use row height count to make detection of invalid row numbers more reliable
- (int)adjustIncompleteIndicatorColumnRowNumbers:(LBZXPDF417BarcodeMetadata *)barcodeMetadata {
  LBZXResultPoint *top = self.isLeft ? self.boundingBox.topLeft : self.boundingBox.topRight;
  LBZXResultPoint *bottom = self.isLeft ? self.boundingBox.bottomLeft : self.boundingBox.bottomRight;
  int firstRow = [self imageRowToCodewordIndex:(int) top.y];
  int lastRow = [self imageRowToCodewordIndex:(int) bottom.y];
...
- (LBZXPDF417BarcodeMetadata *)barcodeMetadata {
  LBZXPDF417BarcodeValue *barcodeColumnCount = [[LBZXPDF417BarcodeValue alloc] init];
  LBZXPDF417BarcodeValue *barcodeRowCountUpperPart = [[LBZXPDF417BarcodeValue alloc] init];
  LBZXPDF417BarcodeValue *barcodeRowCountLowerPart = [[LBZXPDF417BarcodeValue alloc] init];
  LBZXPDF417BarcodeValue *barcodeECLevel = [[LBZXPDF417BarcodeValue alloc] init];
  for (LBZXPDF417Codeword *codeword in self.codewords) {
    if ((id)codeword == [NSNull null]) {
      continue;
    }
  ...
  if (([barcodeColumnCount value].length == 0) ||
      ...
      [barcodeRowCountUpperPart value].array[0] + [barcodeRowCountLowerPart value].array[0] < LBZX_PDF417_MIN_ROWS_IN_BARCODE ||
      [barcodeRowCountUpperPart value].array[0] + [barcodeRowCountLowerPart value].array[0] > LBZX_PDF417_MAX_ROWS_IN_BARCODE) {
    return nil;
  }
  LBZXPDF417BarcodeMetadata *barcodeMetadata = [[LBZXPDF417BarcodeMetadata alloc] initWithColumnCount:[barcodeColumnCount value].array[0]
...
- (void)removeIncorrectCodewords:(LBZXPDF417BarcodeMetadata *)barcodeMetadata {
  // Remove codewords which do not match the metadata
  // TODO Maybe we should keep the incorrect codewords for the start and end positions?
  for (int codewordRow = 0; codewordRow < [self.codewords count]; codewordRow++) {
    LBZXPDF417Codeword *codeword = self.codewords[codewordRow];
