...
#import "LBZXPDF417BarcodeMetadata.h"
#import "LBZXPDF417BarcodeValue.h"
...
#import "LBZXPDF417DetectionResult.h"
#import "LBZXPDF417DetectionResultRowIndicatorColumn.h"
#import "LBZXPDF417ECErrorCorrection.h"
#import "LBZXPDF417ScanningDecoder.h"
#import "LBZXResultPoint.h"

const int LBZX_PDF417_CODEWORD_SKEW_SIZE = 2;

const int LBZX_PDF417_MAX_ERRORS = 3;
const int LBZX_PDF417_MAX_EC_CODEWORDS = 512;
static LBZXPDF417ECErrorCorrection *errorCorrection;

@implementation LBZXPDF417ScanningDecoder

+ (void)initialize {
  errorCorrection = [[LBZXPDF417ECErrorCorrection alloc] init];
}
...
+ (LBZXDecoderResult *)decode:(LBZXBitMatrix *)image
               imageTopLeft:(LBZXResultPoint *)imageTopLeft
            imageBottomLeft:(LBZXResultPoint *)imageBottomLeft
              imageTopRight:(LBZXResultPoint *)imageTopRight
           imageBottomRight:(LBZXResultPoint *)imageBottomRight
           minCodewordWidth:(int)minCodewordWidth
           maxCodewordWidth:(int)maxCodewordWidth {
  LBZXPDF417BoundingBox *boundingBox = [[LBZXPDF417BoundingBox alloc] initWithImage:image topLeft:imageTopLeft bottomLeft:imageBottomLeft topRight:imageTopRight bottomRight:imageBottomRight];
  LBZXPDF417DetectionResultRowIndicatorColumn *leftRowIndicatorColumn;
  LBZXPDF417DetectionResultRowIndicatorColumn *rightRowIndicatorColumn;
  LBZXPDF417DetectionResult *detectionResult;
  for (int i = 0; i < 2; i++) {
    if (imageTopLeft) {
  ...
  for (int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
    ...
    LBZXPDF417DetectionResultColumn *detectionResultColumn;
    if (barcodeColumn == 0 || barcodeColumn == maxBarcodeColumn) {
      detectionResultColumn = [[LBZXPDF417DetectionResultRowIndicatorColumn alloc] initWithBoundingBox:boundingBox isLeft:barcodeColumn == 0];
    } else {
      detectionResultColumn = [[LBZXPDF417DetectionResultColumn alloc] initWithBoundingBox:boundingBox];
    }
    ...
    for (int imageRow = boundingBox.minY; imageRow <= boundingBox.maxY; imageRow++) {
      ...
      LBZXPDF417Codeword *codeword = [self detectCodeword:image minColumn:boundingBox.minX maxColumn:boundingBox.maxX leftToRight:leftToRight
...
+ (LBZXPDF417DetectionResult *)merge:(LBZXPDF417DetectionResultRowIndicatorColumn *)leftRowIndicatorColumn
           rightRowIndicatorColumn:(LBZXPDF417DetectionResultRowIndicatorColumn *)rightRowIndicatorColumn {
  if (!leftRowIndicatorColumn && !rightRowIndicatorColumn) {
    return nil;
  }
  LBZXPDF417BarcodeMetadata *barcodeMetadata = [self barcodeMetadata:leftRowIndicatorColumn rightRowIndicatorColumn:rightRowIndicatorColumn];
  if (!barcodeMetadata) {
    return nil;
  }
  LBZXPDF417BoundingBox *boundingBox = [LBZXPDF417BoundingBox mergeLeftBox:[self adjustBoundingBox:leftRowIndicatorColumn]
                                                              rightBox:[self adjustBoundingBox:rightRowIndicatorColumn]];
  return [[LBZXPDF417DetectionResult alloc] initWithBarcodeMetadata:barcodeMetadata boundingBox:boundingBox];
}

+ (LBZXPDF417BoundingBox *)adjustBoundingBox:(LBZXPDF417DetectionResultRowIndicatorColumn *)rowIndicatorColumn {
  if (!rowIndicatorColumn) {
    return nil;
  }
  LBZXIntArray *rowHeights = [rowIndicatorColumn rowHeights];
  if (!rowHeights) {
    return nil;
  }
...
+ (LBZXPDF417BarcodeMetadata *)barcodeMetadata:(LBZXPDF417DetectionResultRowIndicatorColumn *)leftRowIndicatorColumn
                     rightRowIndicatorColumn:(LBZXPDF417DetectionResultRowIndicatorColumn *)rightRowIndicatorColumn {
  LBZXPDF417BarcodeMetadata *leftBarcodeMetadata;
  ...
  LBZXPDF417BarcodeMetadata *rightBarcodeMetadata;
...
+ (LBZXPDF417DetectionResultRowIndicatorColumn *)rowIndicatorColumn:(LBZXBitMatrix *)image
                                                      boundingBox:(LBZXPDF417BoundingBox *)boundingBox
                                                       startPoint:(LBZXResultPoint *)startPoint
                                                      leftToRight:(BOOL)leftToRight
                                                 minCodewordWidth:(int)minCodewordWidth
                                                 maxCodewordWidth:(int)maxCodewordWidth {
  LBZXPDF417DetectionResultRowIndicatorColumn *rowIndicatorColumn = [[LBZXPDF417DetectionResultRowIndicatorColumn alloc] initWithBoundingBox:boundingBox
                                                                                                                                  isLeft:leftToRight];
  for (int i = 0; i < 2; i++) {
    ...
    for (int imageRow = (int) startPoint.y; imageRow <= boundingBox.maxY &&
      ...
      LBZXPDF417Codeword *codeword = [self detectCodeword:image minColumn:0 maxColumn:image.width leftToRight:leftToRight startColumn:startColumn imageRow:imageRow
                                       minCodewordWidth:minCodewordWidth maxCodewordWidth:maxCodewordWidth];
      if (codeword) {
...
+ (BOOL)adjustCodewordCount:(LBZXPDF417DetectionResult *)detectionResult barcodeMatrix:(NSArray *)barcodeMatrix {
  LBZXIntArray *numberOfCodewords = [(LBZXPDF417BarcodeValue *)barcodeMatrix[0][1] value];
  ...
  if (numberOfCodewords.length == 0) {
    if (calculatedNumberOfCodewords < 1 || calculatedNumberOfCodewords > LBZX_PDF417_MAX_CODEWORDS_IN_BARCODE) {
      return NO;
    }
    [(LBZXPDF417BarcodeValue *)barcodeMatrix[0][1] setValue:calculatedNumberOfCodewords];
...
+ (LBZXDecoderResult *)createDecoderResult:(LBZXPDF417DetectionResult *)detectionResult {
  ...
  LBZXIntArray *codewords = [[LBZXIntArray alloc] initWithLength:detectionResult.barcodeRowCount * detectionResult.barcodeColumnCount];
  ...
  for (int row = 0; row < detectionResult.barcodeRowCount; row++) {
    for (int column = 0; column < detectionResult.barcodeColumnCount; column++) {
      LBZXIntArray *values = [(LBZXPDF417BarcodeValue *)barcodeMatrix[row][column + 1] value];
  ...
  return [self createDecoderResultFromAmbiguousValues:detectionResult.barcodeECLevel
                                            codewords:codewords
                                         erasureArray:[LBZXPDF417Common toIntArray:erasures]
                                     ambiguousIndexes:[LBZXPDF417Common toIntArray:ambiguousIndexesList]
 ...
 */
+ (LBZXDecoderResult *)createDecoderResultFromAmbiguousValues:(int)ecLevel codewords:(LBZXIntArray *)codewords
                                               erasureArray:(LBZXIntArray *)erasureArray ambiguousIndexes:(LBZXIntArray *)ambiguousIndexes
                                       ambiguousIndexValues:(NSArray *)ambiguousIndexValues {
  LBZXIntArray *ambiguousIndexCount = [[LBZXIntArray alloc] initWithLength:ambiguousIndexes.length];

  int tries = 100;
  while (tries-- > 0) {
    for (int i = 0; i < ambiguousIndexCount.length; i++) {
      LBZXIntArray *a = ambiguousIndexValues[i];
      codewords.array[ambiguousIndexes.array[i]] = a.array[(ambiguousIndexCount.array[i] + 1) % [(LBZXIntArray *)ambiguousIndexValues[i] length]];
    }
    LBZXDecoderResult *result = [self decodeCodewords:codewords ecLevel:ecLevel erasures:erasureArray];
    if (result) {
      return result;
    }
    ...
    for (int i = 0; i < ambiguousIndexCount.length; i++) {
      if (ambiguousIndexCount.array[i] < [(LBZXIntArray *)ambiguousIndexValues[i] length] - 1) {
...
+ (NSArray *)createBarcodeMatrix:(LBZXPDF417DetectionResult *)detectionResult {
  ...
  for (int row = 0; row < detectionResult.barcodeRowCount; row++) {
    ...
    for (int column = 0; column < detectionResult.barcodeColumnCount + 2; column++) {
      barcodeMatrix[row][column] = [[LBZXPDF417BarcodeValue alloc] init];
    }
  }

  int column = -1;
  for (LBZXPDF417DetectionResultColumn *detectionResultColumn in [detectionResult detectionResultColumns]) {
    column++;
    if ((id)detectionResultColumn == [NSNull null]) {
      continue;
    }
    for (LBZXPDF417Codeword *codeword in detectionResultColumn.codewords) {
      if ((id)codeword == [NSNull null] || codeword.rowNumber == -1) {
        continue;
      }
      [(LBZXPDF417BarcodeValue *)barcodeMatrix[codeword.rowNumber][column] setValue:codeword.value];
    }
  }
  return barcodeMatrix;
}

+ (BOOL)isValidBarcodeColumn:(LBZXPDF417DetectionResult *)detectionResult barcodeColumn:(int)barcodeColumn {
  return barcodeColumn >= 0 && barcodeColumn <= detectionResult.barcodeColumnCount + 1;
}

+ (int)startColumn:(LBZXPDF417DetectionResult *)detectionResult
     barcodeColumn:(int)barcodeColumn
          imageRow:(int)imageRow
       leftToRight:(BOOL)leftToRight {
  int offset = leftToRight ? 1 : -1;
  LBZXPDF417Codeword *codeword;
  ...
  while ([self isValidBarcodeColumn:detectionResult barcodeColumn:barcodeColumn - offset]) {
    ...
    for (LBZXPDF417Codeword *previousRowCodeword in [detectionResult detectionResultColumn:barcodeColumn].codewords) {
...
+ (LBZXPDF417Codeword *)detectCodeword:(LBZXBitMatrix *)image
  ...
  int endColumn;
  int codewordBitCount = [LBZXPDF417Common bitCountSum:moduleBitCount];
  if (leftToRight) {
  ...
  int decodedValue = [LBZXPDF417CodewordDecoder decodedValue:moduleBitCount];
  int codeword = [LBZXPDF417Common codeword:decodedValue];
  if (codeword == -1) {
    return nil;
  }
  return [[LBZXPDF417Codeword alloc] initWithStartX:startColumn endX:endColumn bucket:[self codewordBucketNumber:decodedValue] value:codeword];
}

+ (NSMutableArray *)moduleBitCount:(LBZXBitMatrix *)image
                         minColumn:(int)minColumn
                         maxColumn:(int)maxColumn
                       leftToRight:(BOOL)leftToRight
                       startColumn:(int)startColumn
...
+ (int)adjustCodewordStartColumn:(LBZXBitMatrix *)image
                       minColumn:(int)minColumn
                       maxColumn:(int)maxColumn
                     leftToRight:(BOOL)leftToRight
  ...
  for (int i = 0; i < 2; i++) {
    while (((leftToRight && correctedStartColumn >= minColumn) || (!leftToRight && correctedStartColumn < maxColumn)) &&
      ...
      if (abs(codewordStartColumn - correctedStartColumn) > LBZX_PDF417_CODEWORD_SKEW_SIZE) {
...
+ (BOOL)checkCodewordSkew:(int)codewordSize minCodewordWidth:(int)minCodewordWidth maxCodewordWidth:(int)maxCodewordWidth {
  return minCodewordWidth - LBZX_PDF417_CODEWORD_SKEW_SIZE <= codewordSize &&
      codewordSize <= maxCodewordWidth + LBZX_PDF417_CODEWORD_SKEW_SIZE;
}

+ (LBZXDecoderResult *)decodeCodewords:(LBZXIntArray *)codewords ecLevel:(int)ecLevel erasures:(LBZXIntArray *)erasures {
  ...
  LBZXDecoderResult *decoderResult = [LBZXPDF417DecodedBitStreamParser decode:codewords ecLevel:[@(ecLevel) stringValue] error:nil];
 ...
 */
+ (int)correctErrors:(LBZXIntArray *)codewords erasures:(LBZXIntArray *)erasures numECCodewords:(int)numECCodewords {
  if (erasures &&
      (erasures.length > numECCodewords / 2 + LBZX_PDF417_MAX_ERRORS ||
       numECCodewords < 0 ||
       numECCodewords > LBZX_PDF417_MAX_EC_CODEWORDS)) {
 ...
 */
+ (BOOL)verifyCodewordCount:(LBZXIntArray *)codewords numECCodewords:(int)numECCodewords {
...
- (NSString *)description:(NSArray *)barcodeMatrix {
  ...
  for (int row = 0; row < [barcodeMatrix count]; row++) {
    ...
    for (int column = 0; column < [(NSArray *)barcodeMatrix[row] count]; column++) {
      LBZXPDF417BarcodeValue *barcodeValue = barcodeMatrix[row][column];
