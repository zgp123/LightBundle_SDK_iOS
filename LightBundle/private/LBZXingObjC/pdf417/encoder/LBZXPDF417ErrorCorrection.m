 ...
 */
const int LBZX_PDF417_EC_COEFFICIENTS[][512] = {
  {27, 917},
  {522, 568, 723, 809},
...
@implementation LBZXPDF417ErrorCorrection
...
+ (NSString *)generateErrorCorrection:(NSString *)dataCodewords errorCorrectionLevel:(int)errorCorrectionLevel {
  ...
  for (int i = 0; i < sld; i++) {
    ...
    int t3;
    for (int j = k - 1; j >= 1; j--) {
      t2 = (t1 * LBZX_PDF417_EC_COEFFICIENTS[errorCorrectionLevel][j]) % 929;
      t3 = 929 - t2;
      e[j] = (unichar) ((e[j - 1] + t3) % 929);
    }
    t2 = (t1 * LBZX_PDF417_EC_COEFFICIENTS[errorCorrectionLevel][0]) % 929;
    t3 = 929 - t2;
    e[0] = (unichar) (t3 % 929);
  }
