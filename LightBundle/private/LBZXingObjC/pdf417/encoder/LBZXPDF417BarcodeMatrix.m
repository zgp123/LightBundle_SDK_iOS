...
@interface LBZXPDF417BarcodeMatrix ()
...
@implementation LBZXPDF417BarcodeMatrix
...
- (id)initWithHeight:(int)height width:(int)width {
  if (self = [super init]) {
    ...
    for (int i = 0, matrixLength = height; i < matrixLength; i++) {
      [matrix addObject:[LBZXPDF417BarcodeRow barcodeRowWithWidth:(width + 4) * 17 + 1]];
    }
    _rowMatrix = matrix;
    _width = width * 17;
    _height = height;
...
- (LBZXPDF417BarcodeRow *)currentRow {
...
- (NSArray *)scaledMatrixWithXScale:(int)xScale yScale:(int)yScale {
  ...
  for (int i = 0; i < yMax; i++) {
    matrixOut[yMax - i - 1] = [(LBZXPDF417BarcodeRow *)self.rowMatrix[i / yScale] scaledRow:xScale];
  }
