...
#import "LBZXPDF417ErrorCorrection.h"
#import "LBZXPDF417HighLevelEncoder.h"

/**
 * The start pattern (17 bits)
 */
const int LBZX_PDF417_START_PATTERN = 0x1fea8;
/**
 * The stop pattern (18 bits)
 */
const int LBZX_PDF417_STOP_PATTERN = 0x3fa29;

/**
 * The codeword table from the Annex A of ISO/IEC 15438:2001(E).
 */
#define LBZX_PDF417_CODEWORD_TABLE_LEN 3
#define LBZX_PDF417_CODEWORD_TABLE_SUB_LEN 929
const int LBZX_PDF417_CODEWORD_TABLE[LBZX_PDF417_CODEWORD_TABLE_LEN][LBZX_PDF417_CODEWORD_TABLE_SUB_LEN] = {
...
const float LBZX_PDF417_PREFERRED_RATIO = 3.0f;
const float LBZX_PDF417_DEFAULT_MODULE_WIDTH = 0.357f; //1px in mm
const float LBZX_PDF417_HEIGHT = 2.0f; //mm

@interface LBZXPDF417 ()

@property (nonatomic, strong) LBZXPDF417BarcodeMatrix *barcodeMatrix;
...
- (id)initWithCompact:(BOOL)compact {
  if (self = [super init]) {
    _compact = compact;
    _compaction = LBZXPDF417CompactionAuto;
    _encoding = LBZX_PDF417_DEFAULT_ENCODING;
    _minCols = 2;
    _maxCols = 30;
    _maxRows = 30;
    _minRows = 2;
...
- (void)encodeCharPattern:(int)pattern len:(int)len logic:(LBZXPDF417BarcodeRow *)logic {
  int map = 1 << (len - 1);
...
- (void)encodeLowLevel:(NSString *)fullCodewords c:(int)c r:(int)r errorCorrectionLevel:(int)errorCorrectionLevel logic:(LBZXPDF417BarcodeMatrix *)logic {
  int idx = 0;
  for (int y = 0; y < r; y++) {
    int cluster = y % 3;
    [logic startRow];
    [self encodeCharPattern:LBZX_PDF417_START_PATTERN len:17 logic:logic.currentRow];
    ...
    int pattern = LBZX_PDF417_CODEWORD_TABLE[cluster][left];
    [self encodeCharPattern:pattern len:17 logic:logic.currentRow];

    for (int x = 0; x < c; x++) {
      pattern = LBZX_PDF417_CODEWORD_TABLE[cluster][[fullCodewords characterAtIndex:idx]];
      [self encodeCharPattern:pattern len:17 logic:logic.currentRow];
      idx++;
    }

    if (self.compact) {
      [self encodeCharPattern:LBZX_PDF417_STOP_PATTERN len:1 logic:logic.currentRow];
    } else {
      pattern = LBZX_PDF417_CODEWORD_TABLE[cluster][right];
      [self encodeCharPattern:pattern len:17 logic:logic.currentRow];

      [self encodeCharPattern:LBZX_PDF417_STOP_PATTERN len:18 logic:logic.currentRow];
    }
  }
}
...
- (BOOL)generateBarcodeLogic:(NSString *)msg errorCorrectionLevel:(int)anErrorCorrectionLevel error:(NSError **)error {
  ...
  int errorCorrectionCodeWords = [LBZXPDF417ErrorCorrection errorCorrectionCodewordCount:anErrorCorrectionLevel];
  NSString *highLevel = [LBZXPDF417HighLevelEncoder encodeHighLevel:msg compaction:self.compaction encoding:self.encoding error:error];
  if (!highLevel) {
    return NO;
  }
  int sourceCodeWords = (int)highLevel.length;

  LBZXIntArray *dimension = [self determineDimensions:sourceCodeWords errorCorrectionCodeWords:errorCorrectionCodeWords error:error];
  if (!dimension) {
    return NO;
  }
  ...
  NSString *ec = [LBZXPDF417ErrorCorrection generateErrorCorrection:dataCodewords errorCorrectionLevel:anErrorCorrectionLevel];
  NSString *fullCodewords = [dataCodewords stringByAppendingString:ec];

  //4. step: low-level encoding
  self.barcodeMatrix = [[LBZXPDF417BarcodeMatrix alloc] initWithHeight:rows width:cols];
...
- (LBZXIntArray *)determineDimensions:(int)sourceCodeWords errorCorrectionCodeWords:(int)errorCorrectionCodeWords error:(NSError **)error {
  ...
  for (int cols = self.minCols; cols <= self.maxCols; cols++) {
    ...
    float newRatio = ((17 * cols + 69) * LBZX_PDF417_DEFAULT_MODULE_WIDTH) / (rows * LBZX_PDF417_HEIGHT);

    // ignore if previous ratio is closer to preferred ratio
    if (dimension && fabsf(newRatio - LBZX_PDF417_PREFERRED_RATIO) > fabsf(ratio - LBZX_PDF417_PREFERRED_RATIO)) {
      continue;
    }

    ratio = newRatio;
    dimension = [[LBZXIntArray alloc] initWithInts:cols, rows, -1];
  }
  ...
  if (!dimension) {
    ...
    if (rows < self.minRows) {
      dimension = [[LBZXIntArray alloc] initWithInts:self.minCols, self.minRows, -1];
    }
  }
